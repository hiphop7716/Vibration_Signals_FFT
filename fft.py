...
for times in range(2):  # 1:x_axis, 2:x&y_axis
    for i in range(len(files)): #read files times according file amount
    # for i in range(1):  # only read file once
        with open(dirPath + "\\" + files[i]) as f:
            data = f.read()
        data = data.split('\n')

        x_axis_g_List = [float(row.split()[times + 1]) for row in data if
        ...
        plt.subplot(312)
        plt.plot(xf_x_float4, yf_x_float4, label=files[i] + ("_X axis" if times + 1 == 1 else "_Y axis"), color='green')
...
baseFreq = -1
# Figure out the highest amplitude of orders that maybe can't be divisible by base frequency but very close.
